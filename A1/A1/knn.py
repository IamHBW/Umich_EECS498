...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            temp = x_train[i].view(-1) - x_test[j].view(-1)
            temp = temp ** 2
            dists[i,j] = temp.sum()
            dists[i,j] = dists[i,j].sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        temp = x_train[i] - x_test
        temp = temp ** 2
        dists[i] = temp.sum(dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.reshape(num_train,-1)
    x_test = x_test.reshape(num_test,-1)
    dists = (x_train ** 2).sum(dim=1,keepdim=True) + (x_test ** 2).sum(dim=1) - 2 * torch.mm(x_train,x_test.t())
    dists = dists.sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        #print("!!!",k)
        dist , indices = dists[:,j].topk(k=k,largest=False) #获取距离最近的前k个点的索引
        labels = []
        for i in range(len(indices)):                   #提取标签值
            labels.append(y_train[indices[i]].item())
        count = torch.zeros(10)
        for label in labels:                #统计标签出现次数
            count[label] += 1
        most_k_count , most_k_labels = count.topk(k=min(10,k))         #找出出现最多次的标签
        y_pred[j] = most_k_labels[0].item()
        #print(count.topk(k=min(10,k)))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_folds = x_train.chunk(num_folds,0)
    y_train_folds = y_train.chunk(num_folds,0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
        ...
        k_to_accuracies[k] = []
        for j in range(num_folds):                #进行交叉比对
            train_empty = True
            jth_x_test = None
            jth_y_test = None
            jth_x_train = None
            jth_y_train = None
            jth_x_test = x_train_folds[j]         #创建验证集
            jth_y_test = y_train_folds[j]
            for i in range(num_folds):                   #创建训练集    连接训练集时出问题了！
                if i != j :
                    #print(i)
                    if train_empty == True:
                        jth_x_train = x_train_folds[i]        
                        jth_y_train = y_train_folds[i]
                        train_empty = False
                    else:
                        jth_x_train = torch.cat([jth_x_train,x_train_folds[i]])
                        jth_y_train = torch.cat([jth_y_train,y_train_folds[i]])
            #print(jth_x_train.shape)
            k_to_accuracies[k].append(KnnClassifier(jth_x_train,jth_y_train).check_accuracy(jth_x_test,jth_y_test,k=k,quiet=False)) 
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
        print(mean(k_to_accuracies[k]))
        if(mean(k_to_accuracies[k])>best_mean):
            best_mean = mean(k_to_accuracies[k])
            best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
