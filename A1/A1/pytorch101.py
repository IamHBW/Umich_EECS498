...
def count_tensor_elements(x: Tensor) -> int:
    ...
    for i in range(len(x.shape)):
        num_elements=num_elements*x.shape[i]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if a>b:
        ...
        x = torch.arange(a,b+1,10,dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    even_rows_odd_cols = x[0:M+1:2,1:N:2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    min_col = y.argmin(dim=1)
    row = torch.arange(y.shape[0])
    y[row,min_col]=0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    temp = temp ** 2
    #print(temp)
    temp2 = temp.sum(dim=0,keepdim=True)
    temp2 /= x.shape[0]-1
    sigma = temp2.sqrt()
    y = (x - mu)/sigma
    ##########################################################################
