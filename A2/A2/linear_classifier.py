...
def svm_loss_vectorized(W, X, y, reg):
  ...
  correct_class_score = scores[y,idx].clone()
  ...
  # Replace "pass" statement with your code
  margin[margin > 0] = 1
  count_pos = margin.clone()
  count_pos = count_pos.sum(dim=0)
  margin[y,idx] = -count_pos
  dW = (margin.mm(X)).t()
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  indices = torch.randint(high=X.shape[0],size=(batch_size,))
  X_batch = X[indices,:]
  y_batch = y[indices]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  result = torch.mm(W.t(),X.t())
  y_pred =torch.argmax(result,dim=0)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  # Replace "pass" statement with your code
  for i in range(5):
    learning_rates.append(1.87e-3 + i * 1e-5)
    regularization_strengths.append(1.87e-5 + i * 1e-7)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  z = torch.mm(W.t(),X.t())
  z -= torch.amax(z,dim=0)
  logits = torch.exp(z)
  prob = logits / logits.sum(dim=0)
  idx = torch.arange(num_train)
  loss = -torch.log(prob[y,idx]).sum().item()
  Y = torch.zeros_like(prob)
  Y[y,idx] = 1
  dW = torch.mm(prob - Y,X).t()
  loss /= num_train
  loss += reg * torch.sum(W * W)
  dW /= num_train
  dW += 2 * reg * W
...
def softmax_get_search_params():
  ...
  # Replace "pass" statement with your code
  for i in range(5):
    learning_rates.append(4.65e-3 + i * 1e-4)
    regularization_strengths.append(1.55e-3 + i * 1e-5)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
