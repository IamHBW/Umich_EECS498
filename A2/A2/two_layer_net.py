...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    log = torch.log(prob[idx,y])
    loss = - log.sum().item() / num_train
    ...
    grad_scores[idx,y] -= 1
    grad_scores /= num_train

    grad_b2 = grad_scores.sum(dim=0)
    hidden = h1.clone()
    h1[h1 < 0] = 0
    grad_W2 = torch.mm(h1.t(),grad_scores)
    grad_relu = torch.mm(grad_scores,W2.t())
    grad_hid = grad_relu
    grad_hid[hidden < 0] = 0
    grad_b1 = grad_hid.sum(dim=0)
    grad_W1 = torch.mm(X.t(),grad_hid)
    grad_W1 += grad_reg1
    grad_W2 += grad_reg2
    grads['W1'] = grad_W1
    grads['W2'] = grad_W2
...
def nn_get_search_params():
  ...
  # Replace "pass" statement with your code
  for i in range(5):
    regularization_strengths.append(1e-4 + i * 1e-5)
    hidden_sizes.append(200 + i * 20)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  ...
  for reg in regularization_strengths:
    for hs in hidden_sizes:
      ...
      stats = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                ...
                learning_rate=learning_rates[0], learning_rate_decay=0.95,
                reg=reg, verbose=False)
      stat_dict[reg] = stats
      y_val_preds = net.predict(data_dict['X_val'])
