...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    F,C,HH,WW = w.shape
    N,C,H,W = x.shape
    out = torch.zeros(N,F,1 + (H + 2 * pad - HH) // stride ,1 + (W + 2 * pad - WW) // stride,dtype=x.dtype,device=x.device)
    for i in torch.arange(N):
      for j in torch.arange(F):
        for m in torch.arange(1 + (H + 2 * pad - HH) // stride):
          for n in torch.arange(0,1 + (W + 2 * pad - WW) // stride):
            h_start = m * stride
            w_start = n * stride
            out[i,j,m,n] = torch.sum(X[i,:,h_start:h_start+HH,w_start:w_start+WW] * w[j,:,:,:]) + b[j] 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
      ...
      for j in torch.arange(F):
        for m in torch.arange(1 + (H + 2 * pad - HH) // stride):
          for n in torch.arange(0,1 + (W + 2 * pad - WW) // stride):
            h_start = m * stride
            w_start = n * stride
            dX[i,:,h_start:h_start+HH,w_start:w_start+WW] += w[j,:,:,:] * dout[i,j,m,n]
            dw[j,:,:,:] += X[i,:,h_start:h_start+HH,w_start:w_start+WW] * dout[i,j,m,n]
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    out = torch.zeros(N,C,1 + (H - pool_height) // stride,1 + (W - pool_width) // stride,dtype=x.dtype,device=x.device)
    for i in torch.arange(N):
      for j in torch.arange(C):
        for m in torch.arange(1 + (H - pool_height) // stride):
          for n in torch.arange(1 + (W - pool_width) // stride):
            h_start = m * stride
            w_start = n * stride
            out[i,j,m,n] = torch.max(x[i,j,h_start:h_start + pool_height,w_start:w_start + pool_width])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
      ...
      for j in torch.arange(C):
        for m in torch.arange(1 + (H - pool_height) // stride):
          for n in torch.arange(1 + (W - pool_width) // stride):
            h_start = m * stride
            w_start = n * stride
            x_ = x[i,j,h_start:h_start + pool_height,w_start:w_start + pool_width]
            max_idx = torch.argmax(x_)
            h_shift = max_idx // pool_width
            w_shift = max_idx % pool_width
            dx[i,j,h_start + h_shift,w_start + w_shift] += dout[i,j,m,n] 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    fc_input_size = num_filters * pooled_H * pooled_W
    self.params['W2'] = torch.randn(fc_input_size,hidden_dim,device=device,dtype=dtype) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    x1,cache1 = Conv_ReLU_Pool.forward(X,W1,b1,conv_param,pool_param)
    N,F,HH,WW = x1.shape
    scores,cache2 = Linear_ReLU.forward(x1.reshape(N,-1),W2,b2)
    ...
    dx,grads['W1'],grads['b1'] = Conv_ReLU_Pool.backward(dx.reshape(N,F,HH,WW),cache1)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    if weight_scale == 'kaiming':
      self.params[f'W{1}'] = kaiming_initializer(C,num_filters[0],3,device=device,dtype=dtype)
      self.params[f'b{1}'] = torch.zeros(num_filters[0],device=device,dtype=dtype)
      if self.batchnorm == True:
        self.params[f'gamma{1}'] = torch.ones_like(self.params[f'b{1}'])
        self.params[f'beta{1}'] = torch.zeros_like(self.params[f'b{1}'])
      for i in range(1,len(num_filters)):
        self.params[f'W{i+1}'] = kaiming_initializer(num_filters[i-1],num_filters[i],3,device=device,dtype=dtype)
        self.params[f'b{i+1}'] = torch.zeros(num_filters[i],device=device,dtype=dtype)
        if self.batchnorm == True:
          self.params[f'gamma{i+1}'] = torch.ones_like(self.params[f'b{i+1}'])
          self.params[f'beta{i+1}'] = torch.zeros_like(self.params[f'b{i+1}'])
      i += 1
      self.pooled_H = H // (2 ** len(max_pools))
      self.pooled_W = W // (2 ** len(max_pools))
      fc_input_size = num_filters[-1] * self.pooled_H * self.pooled_W
      self.params[f'W{i+1}'] = kaiming_initializer(fc_input_size,num_classes,None,device=device,dtype=dtype)
      self.params[f'b{i+1}'] = torch.zeros(num_classes,device=device,dtype=dtype)
    else:
      self.params[f'W{1}'] = torch.randn(num_filters[0],C,3,3,device=device,dtype=dtype) * weight_scale
      self.params[f'b{1}'] = torch.zeros(num_filters[0],device=device,dtype=dtype)
      if self.batchnorm == True:
        self.params[f'gamma{1}'] = torch.ones_like(self.params[f'b{1}'])
        self.params[f'beta{1}'] = torch.zeros_like(self.params[f'b{1}'])
      for i in range(1,len(num_filters)):
        self.params[f'W{i+1}'] = torch.randn(num_filters[i],num_filters[i-1],3,3,device=device,dtype=dtype) * weight_scale
        self.params[f'b{i+1}'] = torch.zeros(num_filters[i],device=device,dtype=dtype)
        if self.batchnorm == True:
          self.params[f'gamma{i+1}'] = torch.ones_like(self.params[f'b{i+1}'])
          self.params[f'beta{i+1}'] = torch.zeros_like(self.params[f'b{i+1}'])
      ...
      self.pooled_W = W // (2 ** len(max_pools))
      fc_input_size = num_filters[-1] * self.pooled_H * self.pooled_W
      self.params[f'W{i+1}'] = torch.randn(fc_input_size,num_classes,device=device,dtype=dtype) * weight_scale
      self.params[f'b{i+1}'] = torch.zeros(num_classes,device=device,dtype=dtype)
  ...
  def loss(self, X, y=None):
    ...
    i += 1
    #print(self.params[f'W{i+1}'].shape,x.reshape(N,-1).shape,i,self.num_layers)
    scores ,cache= Linear.forward(x.reshape(N,-1),self.params[f'W{i+1}'],self.params[f'b{i+1}'])
    caches.append(cache)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for k in range(self.num_layers):
      loss += self.reg * torch.sum(self.params[f'W{k + 1}'] ** 2)

    dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'] = Linear.backward(dout,caches[i])
    grads[f'W{i + 1}'] += 2 * self.reg * self.params[f'W{i + 1}']
    dx = dx.reshape(-1,self.params[f'W{i}'].shape[0],self.pooled_H,self.pooled_W)

    if self.batchnorm == False:
      for i in range(self.num_layers - 2,-1,-1):
        #print(len(caches[i]))
        if i in self.max_pools:
          dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'] = Conv_ReLU_Pool.backward(dx,caches[i])
          
        else:
          dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'] = Conv_ReLU.backward(dx,caches[i])
        grads[f'W{i + 1}'] += 2 * self.reg * self.params[f'W{i+1}']
    else:
      for i in range(self.num_layers - 2,-1,-1):
        #print(len(caches[i]))
        if i in self.max_pools:
          dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'],grads[f'gamma{i + 1}'],grads[f'beta{i + 1}'] = Conv_BatchNorm_ReLU_Pool.backward(dx,caches[i])
          
        else:
          dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'],grads[f'gamma{i + 1}'],grads[f'beta{i + 1}'] = Conv_BatchNorm_ReLU.backward(dx,caches[i])
        grads[f'W{i + 1}'] += 2 * self.reg * self.params[f'W{i+1}']
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  # }
  model = DeepConvNet((3,32,32),[16,32,64,128],[1],weight_scale='kaiming',dtype=dtype,device=device,reg=reg)
  solver = Solver(model,data_dict,
                  print_every=5, num_epochs=20, batch_size=128,
                update_rule=adam,
                optim_config={
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      #######################################################################
      # Replace "pass" statement with your code
      var ,mu = torch.var_mean(x,0,unbiased=False)
      x_hat = (x - mu) / torch.sqrt(var + eps)
      out = gamma * x_hat + beta
      running_mean = momentum * running_mean + (1 - momentum) * mu
      running_var = momentum * running_var + (1 - momentum) * var
      cache = (x_hat,x,gamma,var,mu,eps)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
  ...
  def backward(dout, cache):
    ...
    x_hat,x,gamma,var,mu,eps = cache
    N = dout.shape[0]
    dx_hat = dout * gamma
    dvar = torch.sum(dx_hat * (x - mu),0) * (-0.5) / torch.pow(var + eps,1.5)
    dmu = -torch.sum(dx_hat ,0) / torch.sqrt(var + eps) + dvar * torch.sum(- 2 * (x - mu),0) / N
  ...
  def backward_alt(dout, cache):
    ...
    m = dout.shape[0]
    dx = dout * gamma / std - gamma * x_hat / m / std * torch.sum(dout * x_hat,0) - torch.sum(dout,0) * gamma / std / m
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    # Replace "pass" statement with your code
    N,C,H,W = x.shape
    x_ = x.clone().permute((0,2,3,1)).reshape(-1,C)
    out_,cache = BatchNorm.forward(x_,gamma,beta,bn_param)
    out = out_.clone().reshape(N,H,W,C).permute((0,3,1,2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N,C,H,W = dout.shape
    dout_ = dout.clone().permute((0,2,3,1)).reshape(-1,C)
    dx_,dgamma,dbeta = BatchNorm.backward_alt(dout_,cache)
    dx = dx_.clone().reshape(N,H,W,C).permute((0,3,1,2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
