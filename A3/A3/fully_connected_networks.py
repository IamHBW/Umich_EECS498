...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    layer1 ,cache1 = Linear_ReLU.forward(X,self.params['W1'],self.params['b1'])
    scores,cache2 = Linear.forward(layer1,self.params['W2'],self.params['b2'])
    ...
    loss += self.reg * (torch.sum(self.params['W2'] ** 2) + torch.sum(self.params['W1'] ** 2))
    dlayer1, grads['W2'],grads['b2'] = Linear.backward(dout,cache2) 
    grads['W2'] += 2 * self.reg * self.params['W2']
    _,grads['W1'],grads['b1'] = Linear_ReLU.backward(dlayer1,cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    i += 1
    self.params[f'W{i + 1}'] = torch.randn(hidden_dims[i - 1],num_classes,dtype=dtype,device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    for i in range(0,self.num_layers - 1):
      ...
      if self.use_dropout:
        scores,cache_dp = Dropout.forward(scores,self.dropout_param)
    ...
    for k in range(self.num_layers):
      loss += 0.5 * self.reg * torch.sum(self.params[f'W{k + 1}'] ** 2)
    ...
    while i >= 0:
      if self.use_dropout:
        dx = Dropout.backward(dx,caches[i][1])
        dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'] = Linear_ReLU.backward(dx,caches[i][0])
      else:
        dx,grads[f'W{i + 1}'],grads[f'b{i + 1}'] = Linear_ReLU.backward(dx,caches[i])
