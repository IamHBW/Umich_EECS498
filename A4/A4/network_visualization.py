...
def compute_saliency_maps(X, y, model):
  ...
  scores = model(X)
  loss = torch.gather(scores,1,y.unsqueeze(1)).sum()
  loss.backward()
  saliency = X.grad.data.detach().abs().amax(dim=1)
  ##############################################################################
  #               END OF YOUR CODE                                             #
  ##############################################################################
  return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    score = model(X_adv)
    max_score,label = torch.max(score,1)
    if verbose:
      print(f'Iteration {i}: target score {score[0,target_y].item():.3f}, max score {max_score.item():.3f}')
    if label == target_y:
      print('fooled')
      break
    ...
    with torch.no_grad():
      dX = X_adv.grad
      X_adv += learning_rate * dX / (torch.sqrt(dX ** 2) + 1e-8)
    X_adv.grad.zero_()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    score = model(img)
    loss = score[0,target_y] - l2_reg * torch.sum(img ** 2)
    loss.backward()
    with torch.no_grad():
