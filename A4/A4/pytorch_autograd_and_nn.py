...
def three_layer_convnet(x, params):
  ...
  x = F.relu(F.conv2d(x,conv_w1,conv_b1,padding=2,stride=1))
  x = F.relu(F.conv2d(x,conv_w2,conv_b2,padding=1,stride=1))
  x = flatten(x)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = torch.nn.init.kaiming_normal_(torch.empty( channel_1,C,kernel_size_1,kernel_size_1, dtype=dtype, device=device,requires_grad=True))
  ...
  fc_w = torch.nn.init.kaiming_normal_(torch.empty(num_classes,channel_2 * H * W,dtype=dtype,device=device,requires_grad=True))
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for arg in stage_args:
      stages.append(ResNetStage(*arg,block=block))
  ...
  def forward(self, x):
    ...
    x = self.cnn(x)
    h,w = x.shape[2:]
    pool_layer = nn.AvgPool2d(kernel_size=(h,w))
    x = pool_layer(x)
    x = flatten(x)
    scores = self.fc(x)
    ############################################################################
