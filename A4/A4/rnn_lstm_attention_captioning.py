...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    x,prev_h,out,Wx,Wh = cache

    dout = dnext_h * (1 - torch.tanh(out) ** 2)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.empty(N,step,H,device=x.device,dtype=x.dtype)
    cache = []
    prev_h = h0
    for i in torch.arange(step):
      next_h,step_cache = rnn_step_forward(x[:,i,:],prev_h,Wx,Wh,b)
      h[:,i,:] = next_h
...
def rnn_backward(dh, cache):
    ...
    N,step,H = dh.shape
    D = cache[0][0].shape[1]
    dx = torch.zeros(N,step,D,device=dh.device,dtype=dh.dtype)
    ...
    dprev_h_temp = torch.zeros(N,H,device=dh.device,dtype=dh.dtype)

    for i in torch.arange(step-1,-1,-1):
       dnext_h_total = dh[:,i,:] + dprev_h_temp
       dx_temp, dprev_h_temp, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dnext_h_total,cache[i])
       dx[:,i,:] = dx_temp
       dWx += dWx_temp
       dWh += dWh_temp
       db += db_temp
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        self.outputproj = nn.Linear(hidden_dim,vocab_size,device=device,dtype=dtype)
        self.WordEmbed = WordEmbedding(vocab_size,wordvec_dim,device=device,dtype=dtype)
        if self.cell_type == 'rnn':
          self.featureproj = nn.Linear(input_dim,hidden_dim,device=device,dtype=dtype)
          self.cell = RNN(wordvec_dim,hidden_dim,device=device,dtype=dtype)
          self.FeatureExtractor = FeatureExtractor(pooling=True,device=device,dtype=dtype)
        if  self.cell_type == 'lstm':
           self.featureproj = nn.Linear(input_dim,hidden_dim,device=device,dtype=dtype)
           self.cell = LSTM(wordvec_dim,hidden_dim,device=device,dtype=dtype)
           self.FeatureExtractor = FeatureExtractor(pooling=True,device=device,dtype=dtype)
        if self.cell_type == 'attention':
           self.featureproj = nn.Linear(1280,hidden_dim,device=device,dtype=dtype)
           self.cell = AttentionLSTM(wordvec_dim,hidden_dim,device=device,dtype=dtype)
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        word_vec_in = self.WordEmbed(captions_in)
        feature = self.FeatureExtractor.extract_mobilenet_feature(images)
        if self.cell_type == 'attention':
          A = self.featureproj(feature.permute(0,2,3,1)).permute(0,3,1,2)
          h = self.cell(word_vec_in,A) 
        else:     
          h0 = self.featureproj(feature)
          h = self.cell(word_vec_in,h0)

        scores = self.outputproj(h)
    ...
    def sample(self, images, max_length=15):
        ...
        for t in range(max_length):
          prev_word_vec = self.WordEmbed(prev_word)
          if self.cell_type == 'rnn':            
            next_h = self.cell.step_forward(prev_word_vec,prev_h)            
          elif self.cell_type == 'lstm':
            next_h,next_c = self.cell.step_forward(prev_word_vec,prev_h,prev_c)
            prev_c = next_c
          elif self.cell_type == 'attention':
            attn,attn_weights = dot_product_attention(prev_h,A)
            next_h,next_c = self.cell.step_forward(prev_word_vec,prev_h,prev_c,attn)
            prev_c = next_c
            attn_weights_all[:,t,:,:] = attn_weights
          scores_t = self.outputproj(next_h)
          next_word = torch.argmax(scores_t,1)
          prev_h = next_h
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N,steps,D = x.shape
    H = h0.shape[1]
    h = torch.zeros(N,steps,H,device=x.device,dtype=x.dtype)
    prev_h = h0
    prev_c = c0
...
def dot_product_attention(prev_h, A):
    ...
    # Replace "pass" statement with your code
    A = A.reshape(N,H,16)
    scores = prev_h.unsqueeze(1) @ A 
    scores = scores.permute(0,2,1)
    scores /= torch.sqrt(A.new_tensor(H))
