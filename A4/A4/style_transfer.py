...
def content_loss(content_weight, content_current, content_target):
    ...
    M_l = H_l * W_l
    F_l = content_current.reshape(1,C_l,M_l)
    P_l = content_target.reshape(1,C_l,M_l)
...
def gram_matrix(features, normalize=True):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    _, C_l, H_l, W_l = features.shape
    M_l = H_l * W_l
    features = features.reshape(-1,C_l,M_l)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      idx = style_layers[i]
      gram_i = gram_matrix(feats[idx])
      style_loss += style_weights[i] * torch.sum((gram_i - style_targets[i]) ** 2)
    return style_loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
