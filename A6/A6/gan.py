...
def discriminator_loss(logits_real, logits_fake):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  true_labels = torch.ones(logits_fake.shape[0],device=logits_fake.device,dtype=logits_fake.dtype)
  false_labels = torch.zeros(logits_fake.shape[0],device=logits_fake.device,dtype=logits_fake.dtype)
  loss_x = nn.functional.binary_cross_entropy_with_logits(logits_real.squeeze(),true_labels,reduction='mean')
  loss_z = nn.functional.binary_cross_entropy_with_logits(logits_fake.squeeze(),false_labels,reduction='mean')
  loss = loss_x + loss_z
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
...
def generator_loss(logits_fake):
  ...
  # Replace "pass" statement with your code
  true_labels = torch.ones(logits_fake.shape[0],device=logits_fake.device,dtype=logits_fake.dtype)
  loss = nn.functional.binary_cross_entropy_with_logits(logits_fake.squeeze(),true_labels,reduction='mean')
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
  return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
  ...
  loss = torch.sum((scores_real - 1) ** 2) / 2 / N + torch.sum(scores_fake ** 2) / 2 / N
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
  return loss
...
def ls_generator_loss(scores_fake):
  ...
  loss = torch.sum((scores_fake - 1) ** 2 ) / 2 / N  
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
  return loss
